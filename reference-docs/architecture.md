...
- Backend detection is cached at startup to avoid a 3-second Ollama probe on every request; refreshed via `/api/status`
...
- If an LLM call fails mid-pipeline, the failure is recorded as an audit event (not silently dropped) and the error is propagated to the user
...
### 3. Consistency Checker (`glass/verifier.py`)

Takes the list of claims and assesses each one for internal coherence.

- For each claim, the checker attempts to determine if it's consistent, uncertain, or unverifiable
- Checking strategies (applied in order):
...
- Each claim gets a tag: `consistent`, `uncertain`, or `unverifiable`
- Each claim gets a short evidence string explaining the tag
- The checker also examines the user's original query for false premises and flags them
- The consistency-check LLM call is recorded in the audit trail
- Important: the checker uses the same type of LLM as the generator. "Consistent" means internally coherent, not factually verified against external sources.

### 4. Audit Trail (`glass/audit.py`)

Captures every operation the system performs during a query lifecycle.

- Each audit event includes: timestamp, operation type, backend used, latency in milliseconds, payload sizes (bytes sent/received), and a full SHA-256 hash of request/response content
...
Takes the consistency-checked response and presents it in the browser with a **process-first layout**.
...
- The answer section displays the response with inline claim annotations (color-coded by consistency status).
- Each claim is clickable — expanding it shows the evidence and checking reasoning.
- A header bar shows the overall consistency summary (e.g., "5 consistent, 2 uncertain, 1 unverifiable") plus total pipeline time and data flow totals.
...
```
{
  ...
  "content_hash": string,     // Full SHA-256 of the response content
